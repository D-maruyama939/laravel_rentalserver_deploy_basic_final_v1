...
@section('content')
    ...
    @if($key_word === '' && $prefecture === '' && $tags === '' && $post_id === '')
        <h1 class="mb-2">おすすめの投稿</h1>
    @else
        <h1 class="mb-2">検索結果</h1>
        
        {{-- 検索インデックス --}}
        <div class="mb-2">
            @if($prefecture !== '')
                【 <i class="fas fa-map-marked-alt"></i> {{ $prefecture->prefecture }} 】
            @endif
            ...
            <div class="card-footer">
                <div class="d-flex justify-content-between">
                    <div class="d-flex flex-row bd-highlight">
                        ...
                        {{-- Line共有ボタン --}}
                        <div class="pl-3 pt-2">
                            <div class="line-it-button" data-lang="ja" data-type="share-a" data-env="REAL" data-url="https://6a95c3488fb8488891d8e4217de80605.vfs.cloud9.us-east-1.amazonaws.com/posts?post_id={{ $post->id }}" data-color="default" data-size="small" data-count="false" data-ver="3" style="display: none;"></div>
                        </div>
                    </div>
                    ...
                    @if(Auth::user()->id === $post->user_id)
                        <div>
                            ...
                            <a class="btn btn-secondary btn-sm text-light" data-toggle="modal" data-target="#modal_{{ $post->id }}">
    ...
    @empty
        <div class="mb-5">投稿がありません</div>
    @endforelse
    
    
    {{-- 検索がなければタイムラインを表示 --}}
    @if($key_word === '' && $prefecture === '' && $tags === '' && $post_id === '')
        <div class="h2 mt-5">タイムライン</div>
        {{-- 投稿を表示 --}}
        @forelse($time_line_posts as $post)
            {{-- 投稿をカードで表示 --}}
            <div class="card post">
                {{-- カードヘッダー --}}
                <div class="card-header">
                ...
                <div class="card-body"> 
                    {{-- 投稿の各スポットをカルーセルで表示 --}}
                    <div id="cl_tl_{{ $loop->index }}" class="carousel slide" data-ride="false"> {{-- $loop->index :現在のループのインデックス --}}
                        {{-- インジゲーター --}}
                        <ol class="carousel-indicators">
                            @foreach($post->spots as $spot)
                                <li
                                    data-target="#cl_tl_{{ $loop->parent->index }}" {{-- $loop->parent->index :現在のループの親ループのインデックス --}}
                                    data-slide-to="tl_{{ $loop->index }}" 
                                    @if($loop->first) {{-- $loop->first :現在のループのインデックスが0ならtrue --}}
                                        class="active"
                                    @endif
                                ></li>
                        ...
                        {{-- previous(前に戻る),同様にnext(次へ進む）ボタン --}}
                        <a class="carousel-control-prev" href="#cl_tl_{{ $loop->index }}" data-slide="prev">
                            <span class="carousel-control-prev-icon"></span>
                        </a>
                        <a class="carousel-control-next" href="#cl_tl_{{ $loop->index }}" data-slide="next">
                            <span class="carousel-control-next-icon"></span>
                        </a>
                    </div>
                </div>
